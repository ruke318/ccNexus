...
func main() {
	...
	if cfg.GetClaudePort() == cfg.GetCodexPort() {
		logger.Info("ccNexus headless API listening on :%d (data dir: %s, db: %s)", cfg.GetClaudePort(), dataDir, dbPath)
	} else {
		logger.Info("ccNexus headless API listening on Claude:%d Codex:%d (data dir: %s, db: %s)", cfg.GetClaudePort(), cfg.GetCodexPort(), dataDir, dbPath)
	}
...
func applyEnvOverrides(cfg *config.Config) {
	if portStr := os.Getenv("CCNEXUS_CLAUDE_PORT"); portStr != "" {
		if port, err := strconv.Atoi(portStr); err == nil {
			cfg.UpdateClaudePort(port)
		} else {
			logger.Warn("Invalid CCNEXUS_CLAUDE_PORT value %q: %v", portStr, err)
		}
	}
	if portStr := os.Getenv("CCNEXUS_CODEX_PORT"); portStr != "" {
		if port, err := strconv.Atoi(portStr); err == nil {
			cfg.UpdateCodexPort(port)
		} else {
			logger.Warn("Invalid CCNEXUS_CODEX_PORT value %q: %v", portStr, err)
		}
	}
